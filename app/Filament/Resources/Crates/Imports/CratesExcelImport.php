...
class CratesExcelImport implements SkipsEmptyRows, SkipsOnFailure, ToModel, WithBatchInserts, WithChunkReading, WithHeadingRow, WithValidation
{
    ...
    {
        // Skip row if crate_id is missing or empty
        if (empty($row['crate_id'])) {
            return null;
        }
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $failure) {
            \Log::error("Import Validation Failure at row {$failure->row()}: ".json_encode($failure->errors()));
            \Log::debug('Failure Row Data: '.json_encode($failure->values()));
        }
    }
}
